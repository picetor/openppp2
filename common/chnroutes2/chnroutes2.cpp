...
bool chnroutes2_equals(const ppp::set<ppp::string>& news, const ppp::set<ppp::string>& olds) noexcept {
    std::size_t news_count = news.size();
    if (news_count != olds.size()) {
        return false;
    }
    ...
    for (const ppp::string& key : news) {
        auto tail = olds.find(key);
        auto endl = olds.end();
        if (tail == endl) {
            return false;
        }
