...
原版代码的完整编译环境、第三方依赖库构建、GLIBC 兼容层、CMake 配置及构建步骤，请参见：
> 📄 **[`releases环境需求清单.md`](releases环境需求清单.md)** — 原版发布版编译环境清单（关闭调试日志）

### WSS 修改版清单【已修改代码】

本 `master` 分支修改版的编译环境清单，适配完全静态链接、GLIBC 兼容层、条件编译、多变体构建等特性，请参见：
> 📄 **[`WSS修改版环境需求.md`](WSS修改版环境需求.md)** — WSS 修改版编译环境清单（保留调试日志）
