...
fun String.unwrapIDN(): String {
    if (isIPAddress(this) || this.all { it.code < 128 }) {
        return this
    }
    return try {
...
fun isIPAddress(host: String): Boolean {
    ...
    return try {
        // Pure IP literals are resolved without a DNS lookup.
...
fun joinHostPort(host: String, port: Int): String {
    if (host.contains(":") && isIPAddress(host)) {
        return "[$host]:$port"
    }
    return "$host:$port"
}
...
fun isHTTPorHTTPSURL(url: String): Boolean {
    return try {
        ...
        (u.scheme == "http" || u.scheme == "https") && u.host != null
    } catch (_: Exception) {
        false
    }
}
