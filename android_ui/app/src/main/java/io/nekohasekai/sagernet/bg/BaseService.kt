...
class BaseService {
    ...
    class Data internal constructor(private val service: Interface) {
        ...
        var proxy: OpenPPP2Instance? = null
    ...
    class Binder(private var data: Data? = null) : ISagerNetService.Stub(),
                ...
                if (data?.state == State.Connected && bandwidthListeners.isNotEmpty()) {
                    broadcast { item ->
                        if (bandwidthListeners.contains(item.asBinder())) {
                            ...
                            outs.forEach { (profileId, subStats) ->
                                item.trafficUpdated(
                                    profileId, TrafficStats(
                                        txRateDirect = subStats.txTotal,
                                        rxTotal = subStats.rxTotal
                                    ), false
                                )
                            }
                        }
        ...
        // Per-app traffic stats are not provided by the openppp2 core;
        // loopStats stays as a no-op that simply terminates when asked.
        private suspend fun loopStats() {
            if (statsListeners.isEmpty()) return
            val delayMs = statsListeners.values.minOrNull() ?: return
            if (delayMs == 0L) return
            // The core does not export per-app counters; publish an empty list.
    ...
    interface Interface {
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            val proxy = OpenPPP2Instance(profile, this as Service)
            data.proxy = proxy
