...
> [Android overview](../../../../../../README.md) · [Technical guide](../../../../../../debug_CN.md)
...
|---|---|
| `rules/geo-rules.txt` | `files/rules/geo-rules.txt` |
...
The service also sets the native root path to Android `filesDir`, allowing
relative runtime configuration paths such as `./rules/geo-rules.txt`,
`./rules/GeoIP.dat`, and `./rules/GeoSite.dat` to resolve to the extracted
files. When GEO mode is enabled, the fork-specific JNI bridge passes these
three paths to the picetor core's `GeoRuleEngine`.

## Maintenance rules

- Keep the APK asset filenames lowercase.
...
For the Android client lifecycle and diagnostics, see the [technical guide](../../../../../../debug_CN.md).
