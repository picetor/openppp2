...
class PppVpnService : VpnService() {
    ...
    private fun ensureGeoRulesAssets() {
        try {
            ...
            for ((assetPath, destName) in pairs) {
                ...
                // The routing policy is application-owned and may change
                // between APK builds even when the GeoIP/GeoSite datasets do
                // not. Refresh it on every service creation so an upgrade
                // cannot keep an obsolete first-install rule order forever.
                val refreshBundledPolicy = destName == "geo-rules.yaml"
            ...
            // Remove the pre-YAML filename so an upgrade cannot leave a stale
            // copy that no longer participates in routing and confuses dumps.
            val legacyRules = java.io.File(rulesDir, "geo-rules.txt")
            if (legacyRules.exists()) {
                legacyRules.delete()
                PppLog.write(this, "removed legacy geo-rules.txt")
            }
            for (name in listOf("geo-rules.yaml", "GeoIP.dat", "GeoSite.dat")) {
    ...
    /**
     * Materialize the GEO policy file consumed by the native GeoRuleEngine.
     * The selected country is direct (physical network); every unmatched
     * destination falls through to the single active tunnel. This is the same
     * single-outbound policy shape the old client shipped, restored for the
     * local ppp core which never had Miaocchi's GeoRuleGenerator.
     *
    ...
    private fun writeGeoRulesPreset(
        ...
        return try {
            ...
            // Same YAML schema as the desktop `geo-rules.yaml`. The native
            // GeoRuleEngine detects the .yaml extension and uses the shared
            // YAML 1.2 subset parser, so Android and desktop behave identically.
            val rulesFile = java.io.File(rulesDir, "geo-rules.yaml")
            val dnsList = if (customRules.isNotBlank()) {
                // User-authored rules take precedence; the effective DNS list is
                // whatever the custom YAML declares, so only report the fallback.
                emptyList<String>()
            } else if (directDns.isNotEmpty()) {
                directDns
            } else if (country == "cn") {
            ...
            val content = if (customRules.isNotBlank()) {
                ...
                buildString {
                    ...
                    appendLine("# The selected country is direct; all unmatched traffic uses the tunnel.")
    ...
    private fun startVpn(configJson: String, vpnOptionsJson: String) {
        ...
        try {
            ...
            // ---- GEO routing: selected country is direct, everything else
            // uses the single active tunnel. The preset policy file is
            // regenerated on every connect from the profile's `geoRules`
            // block and handed to the native GeoRuleEngine via set_geo_rules.
            ...
            if (geoEnabled) {
                ...
                val geoResult = libopenppp2.set_geo_rules(
                    "./rules/geo-rules.yaml",
                    "./rules/GeoSite.dat",
                    "./rules/GeoIP.dat"
                )
                PppLog.write(
                    this,
                    "set_geo_rules country=$geoCountry enabled=true result=$geoResult"
                )
                if (!geoResult) {
                ...
                PppLog.write(this, "geo rules disabled (proxyOnly=$proxyOnly enabled=${geoRules?.optBoolean("enabled", false)})")
            }
