...
#pragma warning(push)
#pragma warning(disable: 4996) // C4996: 'deadline_timer': Use system_timer
...
// 全局禁用 C4996，因为 Boost 1.91 中 deadline_timer 被标记为 [[deprecated]]
// 但代码中广泛使用，且 VS 2026 将 [[deprecated]] 属性视为 C4996 错误
...
#include <WinSock2.h>
#else
// Boost 1.87.0 removed io_service; io_context is the native type.
...
#else
#if defined(_WIN32)
#define LOG_TAG(TAG, FORMAT, ...)   do { ::fprintf(stdout, "[%s][" TAG "](%s:%d): " FORMAT "\r\n", ((char*)::ppp::GetCurrentTimeText<ppp::string>().data()), __FILE__, __LINE__, __VA_ARGS__); ::fflush(stdout); } while (0)
#else   
#define LOG_TAG(TAG, FORMAT, ...)   do { ::fprintf(stdout, "[%s][" TAG "](%s:%d): " FORMAT "\r\n", ((char*)::ppp::GetCurrentTimeText<ppp::string>().data()), __FILE__, __LINE__, __VA_ARGS__); ::fflush(stdout); } while (0)
#endif
