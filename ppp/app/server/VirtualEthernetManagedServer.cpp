    ...
    namespace app {
        namespace server {
            ...
            VirtualEthernetManagedServer::IWebScoketPtr VirtualEthernetManagedServer::NewWebSocketConnectToManagedServer(const ppp::string& url, YieldContext& y) noexcept {
                ...
                auto& client_cfg = configuration_->client.websocket;
                ...
                bool running = websocket->Run(WebSocket::HandshakeType_Client, ws_host, sni, path, y);
                if (!running) {
                    return NULLPTR;
                }
            ...
            bool VirtualEthernetManagedServer::IWebSocket::Run(HandshakeType type, const ppp::string& host, const ppp::string& path, YieldContext& y) noexcept {
                return Run(type, host, host, path, y);
            }

            bool VirtualEthernetManagedServer::IWebSocket::Run(HandshakeType type, const ppp::string& host, const ppp::string& sni, const ppp::string& path, YieldContext& y) noexcept {
                if (auto p = ws; NULLPTR != p) {
                ...
                if (auto p = wss; NULLPTR != p) {
                    ...
                    return p->Run(type, host, sni, path, verify_peer,
