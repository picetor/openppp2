        ...
        namespace server {
            ...
            void VirtualEthernetExchanger::Finalize() noexcept {
                ...
                for (;;) {
                    ...
                    for (auto& [id, conn] : tcp_connections_) {
                        if (NULLPTR != conn && NULLPTR != conn->Socket) {
                            ppp::net::Socket::Closesocket(conn->Socket);
                        }
                    }
            ...
            bool VirtualEthernetExchanger::OnConnect(const ITransmissionPtr& transmission, int connection_id, const boost::asio::ip::tcp::endpoint& destinationEP, YieldContext& y) noexcept {
                ...
                if (destinationPort <= IPEndPoint::MinPort || destinationPort > IPEndPoint::MaxPort) {
                    DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_CONNECT_TO_DESTINATION, y);
                    return true; // keep Run() alive
                }

                boost::asio::ip::address destinationIP = destinationEP.address();
                if (destinationIP.is_unspecified() || destinationIP.is_multicast()) {
                    DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_CONNECT_TO_DESTINATION, y);
                    return true; // keep Run() alive
                }

                // Log TCP connect attempt
                VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                if (NULLPTR != logger) {
                    ppp::string dst = Ipep::ToAddressString<ppp::string>(destinationEP);
                    logger->TcpConnect(GetId(), connection_id, dst, destinationPort);
                }

                // Check firewall
                FirewallPtr firewall = firewall_;
                if (NULLPTR != firewall) {
                    if (firewall->IsDropNetworkPort(destinationPort, true)) {
                        DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_CONNECT_TO_DESTINATION, y);
                        return true; // keep Run() alive
                    }
                ...
                ppp::net::Socket::AdjustDefaultSocketOptional(handle, destinationIP.is_v4());
                ...
                if (NULLPTR != configuration) {
                    ppp::net::Socket::SetWindowSizeIfNotZero(handle, configuration->tcp.cwnd, configuration->tcp.rwnd);
                    ppp::net::Socket::AdjustSocketOptional(*socket, destinationIP.is_v4(), configuration->tcp.fast_open, configuration->tcp.turbo);
                }

                // Connect to destination
                bool connected = ppp::coroutines::asio::async_connect(*socket, destinationEP, y);
                if (disposed_) {
                    DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_CONNECT_CANCEL, y);
                    return false;
                }
                ...
                std::shared_ptr<Byte> buffer = ppp::threading::BufferswapAllocator::MakeByteArray(allocator, PPP_BUFFER_SIZE);
                if (NULLPTR == buffer) {
                    DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_CONNECT_TO_DESTINATION, y);
                    return true; // keep Run() alive
                }
                ...
                // Store connection
                auto conn = make_shared_object<TcpConnection>();
                if (NULLPTR == conn) {
                    DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_CONNECT_TO_DESTINATION, y);
                    return true; // keep Run() alive
                }
                ...
                tcp_connections_[connection_id] = conn;

                // Send CONNECT_OK to client
                bool ok = DoConnectOK(transmission, connection_id, ERROR_CODES::ERRORS_SUCCESS, y);
                if (!ok) {
                    DeleteTcpConnection(connection_id);
                    return true; // keep Run() alive
                }

                // Start receiving data from socket -> transmission
                ReceiveTcpConnection(conn, connection_id);

                // Log successful TCP connect
                if (NULLPTR != logger) {
                    ppp::string dst = Ipep::ToAddressString<ppp::string>(destinationEP);
                    logger->TcpConnect(GetId(), connection_id, dst + ":CONNECTED", destinationPort);
                }

                // OnConnect returns true to keep the Run() loop alive.
                // Data from transmission will be delivered via OnPush().
                return true;
            }
            ...
            bool VirtualEthernetExchanger::OnPush(const ITransmissionPtr& transmission, int connection_id, Byte* packet, int packet_length, YieldContext& y) noexcept {
                ...
                if (connection_id == 0 || NULLPTR == packet || packet_length < 1) {
                    return true; // ignore malformed packets, keep Run() alive
                }

                auto it = tcp_connections_.find(connection_id);
                if (it == tcp_connections_.end()) {
                    return true; // connection already closed, keep Run() alive
                }

                std::shared_ptr<TcpConnection> conn = it->second;
                if (NULLPTR == conn || NULLPTR == conn->Socket || !conn->Connected) {
                    return true; // connection already closed, keep Run() alive
                }

                std::shared_ptr<boost::asio::ip::tcp::socket> socket = conn->Socket;
                bool ok = ppp::coroutines::asio::async_write(*socket, boost::asio::buffer(packet, packet_length), y);
                if (!ok) {
                    VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                    if (NULLPTR != logger) {
                        logger->TcpDisconnect(GetId(), connection_id, "push_write_failed");
                    }
                    DeleteTcpConnection(connection_id);
                    return true; // write failed but keep Run() alive for other connections
                }

                // Log forwarded data
                VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                if (NULLPTR != logger) {
                    logger->TcpForward(GetId(), connection_id, packet_length);
                }
            ...
            bool VirtualEthernetExchanger::OnDisconnect(const ITransmissionPtr& transmission, int connection_id, YieldContext& y) noexcept {
                ...
                VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                if (NULLPTR != logger) {
                    logger->TcpDisconnect(GetId(), connection_id, "client_fin");
                }
            ...
            bool VirtualEthernetExchanger::OnMux(const ITransmissionPtr& transmission, uint16_t vlan, uint16_t max_connections, bool acceleration, Byte ordering_caps, YieldContext& y) noexcept {
                ...
                AppConfigurationPtr configuration = switcher_->GetConfiguration();
                ...
                for (;;) {
                    ...
                    mux = make_shared_object<vmux::vmux_net>(vmux_context, vmux_strand,
                        max_connections, true, acceleration, mux_mode);
                    if (NULLPTR != mux) {
                        mux->Vlan = vlan;
                        ...
                        // A flow-mode server accepts the client's optional turbo
                        // carrier growth up to the same bounded hard ceiling.
            ...
            bool VirtualEthernetExchanger::Open() noexcept {
                ...
                VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                if (NULLPTR != logger) {
                    ppp::string remote = ppp::net::IPEndPoint::ToEndPoint(transmission->GetRemoteEndPoint()).ToString();
                    logger->ModuleStart("VirtualEthernetExchanger", BOOST_BEAST_VERSION_STRING, "REMOTE:" + remote);
                }
            ...
            bool VirtualEthernetExchanger::DeleteTcpConnection(int connection_id) noexcept {
                if (connection_id == 0) {
                    return false;
                }
                ...
                std::shared_ptr<TcpConnection> conn = std::move(it->second);
                tcp_connections_.erase(it);

                if (NULLPTR != conn) {
                    conn->Connected = false;
                    if (NULLPTR != conn->Socket) {
                        VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                        if (NULLPTR != logger) {
                            logger->TcpDisconnect(GetId(), connection_id, "connection_cleanup");
                        }
                        ppp::net::Socket::Closesocket(conn->Socket);
                    }
                }
            ...
            bool VirtualEthernetExchanger::ForwardTcpConnection(const ITransmissionPtr& transmission, int connection_id, YieldContext& y) noexcept {
                if (disposed_) {
                    return false;
                }
                if (NULLPTR == transmission) {
                ...
                while (!disposed_) {
                    ...
                    std::shared_ptr<boost::asio::ip::tcp::socket> socket = conn->Socket;
                    bool ok = ppp::coroutines::asio::async_write(*socket, boost::asio::buffer(packet.get(), packet_length), y);
                    if (!ok) {
                        DeleteTcpConnection(connection_id);
                        return false;
                    }
            ...
            bool VirtualEthernetExchanger::ReceiveTcpConnection(const std::shared_ptr<TcpConnection>& conn, int connection_id) noexcept {
                if (disposed_) {
                    return false;
                }
                if (NULLPTR == conn || NULLPTR == conn->Socket || NULLPTR == conn->Buffer) {
                    return false;
                }
                if (!conn->Connected) {
                    return false;
                ...
                auto self = shared_from_this();
                std::shared_ptr<boost::asio::ip::tcp::socket> socket = conn->Socket;
                std::shared_ptr<Byte> buffer = conn->Buffer;

                socket->async_read_some(boost::asio::buffer(buffer.get(), PPP_BUFFER_SIZE),
                    [self, this, socket, buffer, connection_id](const boost::system::error_code& ec, std::size_t sz) noexcept {
                        int bytes_transferred = static_cast<int>(sz);
                        if (ec || bytes_transferred < 1) {
                            // Connection closed or error, send FIN to client
                            // Construct FIN packet: [PacketAction_FIN(1)][connection_id(3)]
                            ITransmissionPtr t = transmission_;
                            if (NULLPTR != t) {
                                Byte fin_packet[4] = {
                                    static_cast<Byte>(0x2D), // PacketAction_FIN
                                    static_cast<Byte>(connection_id >> 16),
                                    static_cast<Byte>(connection_id >> 8),
                                    static_cast<Byte>(connection_id)
                                };
                                t->Write(fin_packet, sizeof(fin_packet), nullptr);
                            }
                            VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                            if (NULLPTR != logger) {
                                logger->TcpDisconnect(GetId(), connection_id, ec ? ec.message().c_str() : "remote_closed");
                            }
                            DeleteTcpConnection(connection_id);
                            return;
                        }

                        // Log received data from remote
                        VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                        if (NULLPTR != logger) {
                            logger->TcpForward(GetId(), connection_id, bytes_transferred);
                        }

                        // Forward data to client via transmission
                        ITransmissionPtr transmission = transmission_;
                        if (NULLPTR != transmission) {
                            transmission->Write(buffer.get(), bytes_transferred,
                                [self, this, buffer, connection_id](bool ok) noexcept {
                                    if (ok) {
                                        // Look up connection again (may have been removed)
                                        auto it = tcp_connections_.find(connection_id);
                                        if (it != tcp_connections_.end()) {
                                            ReceiveTcpConnection(it->second, connection_id);
                                        }
                                    }
                                    else {
                                        VirtualEthernetLoggerPtr logger = switcher_->GetLogger();
                                        if (NULLPTR != logger) {
                                            logger->TcpDisconnect(GetId(), connection_id, "forward_write_failed");
                                        }
                                        DeleteTcpConnection(connection_id);
                                    }
                                });
