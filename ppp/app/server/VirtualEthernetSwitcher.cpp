        ...
        namespace server {
            ...
            int VirtualEthernetSwitcher::Run(const ContextPtr& context, const ITransmissionPtr& transmission, YieldContext& y) noexcept {
                ...
                if (session_id == 0) {
                    ...
                    if (NULLPTR != logger) {
                        logger->Handshake(0, transmission, "", "", "", false);
                    }
                    return STATUS_ERROR;
                }
                ...
                // Log handshake success
                {
                    VirtualEthernetLoggerPtr logger = GetLogger();
                    if (NULLPTR != logger) {
                        logger->Handshake(session_id, transmission, "", "", "", true);
                    }
                }
            ...
            bool VirtualEthernetSwitcher::Open(const ppp::string& firewall_rules) noexcept {
                ...
                if (ok) {
                    ...
                    VirtualEthernetLoggerPtr logger = GetLogger();
                    if (NULLPTR != logger) {
                        logger->ModuleStart("VirtualEthernetSwitcher", BOOST_BEAST_VERSION_STRING, "");
                    }
                }
