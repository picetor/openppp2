...
namespace ppp {
    namespace app {
        namespace protocol {
            ...
            bool VirtualEthernetLogger::Handshake(Int128 guid, const std::shared_ptr<ppp::transmissions::ITransmission>& transmission, const ppp::string& host, const ppp::string& sni, const ppp::string& path, bool ok) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += LOGGER_GUID(guid) + " ";
                log += "HANDSHAKE ";
                log += (ok ? "OK" : "FAILED");
                ...
                if (NULLPTR != transmission) {
                    log += " REMOTE:";
                    log += ppp::net::IPEndPoint::ToEndPoint(transmission->GetRemoteEndPoint()).ToString();
                }
                log += "\r\n";
            ...
            bool VirtualEthernetLogger::ModuleStart(const ppp::string& module_name, const ppp::string& version, const ppp::string& extra) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += "MODULE START ";
                log += module_name;
                log += " VERSION:";
            ...
            bool VirtualEthernetLogger::TcpConnect(Int128 guid, int connection_id, const ppp::string& destination, int port) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += LOGGER_GUID(guid) + " ";
                log += "TCP CONNECT ID:";
                log += stl::to_string<ppp::string>(connection_id);
                log += " DESTINATION:";
                log += destination;
                log += ":";
                log += stl::to_string<ppp::string>(port);
                log += "\r\n";

                return this->Write(log.data(), log.size(), NULLPTR);
            }

            bool VirtualEthernetLogger::TcpDisconnect(Int128 guid, int connection_id, const ppp::string& reason) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += LOGGER_GUID(guid) + " ";
                log += "TCP DISCONNECT ID:";
                log += stl::to_string<ppp::string>(connection_id);
                log += " REASON:";
                log += reason;
                log += "\r\n";
            ...
            bool VirtualEthernetLogger::TcpForward(Int128 guid, int connection_id, int bytes) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += LOGGER_GUID(guid) + " ";
                log += "TCP FORWARD ID:";
                log += stl::to_string<ppp::string>(connection_id);
                log += " BYTES:";
                log += stl::to_string<ppp::string>(bytes);
                log += "\r\n";

                return this->Write(log.data(), log.size(), NULLPTR);
            }

            bool VirtualEthernetLogger::Info(const ppp::string& message) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += "INFO ";
                log += message;
                log += "\r\n";
            ...
            bool VirtualEthernetLogger::Warn(const ppp::string& message) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += "WARN ";
                log += message;
                log += "\r\n";
            ...
            bool VirtualEthernetLogger::Error(const ppp::string& message) noexcept {
                ppp::string log = LOGGER_NOW() + " ";
                log += "ERROR ";
                log += message;
                log += "\r\n";
