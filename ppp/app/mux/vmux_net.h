...
#pragma pack(push, 1)
         ...
         *   - seq           (4 bytes) 鈥?monotonically increasing frame sequence number.
         *   - cmd           (1 byte)  鈥?vmux command identifier (see anonymous enum below).
         *   - connection_id (4 bytes) 鈥?logical connection this frame belongs to.
         *
...
#pragma pack(pop)
         ...
         */
        enum {
            cmd_none         = ('E' - 1), ///< Sentinel 鈥?no command / uninitialized.
            cmd_syn,                      ///< SYN 鈥?request to open a new logical connection.
            cmd_syn_ok,                   ///< SYN-OK 鈥?server acknowledges the connection request.
            cmd_push,                     ///< PUSH 鈥?carry application payload.
            cmd_fin,                      ///< FIN 鈥?close the logical connection gracefully.
    ...
    public:
         ...
         *          proxy 鈥?worse quality => larger pool, per the design) within
    ...
    public:
        ...
        bool                                                                        do_yield(ppp::coroutines::YieldContext& y, YieldHandler&& h) noexcept {
            ...
            // park the coroutine with no future Resume() 鈥?a permanent coroutine leak.
    ...
    private:
         ...
         *           (last_active_), NOT RTT 鈥?a cheap signal that reuses existing
         ...
         *  sends the next queued frame 鈥?no per-connection binding); the difference
    ...
    private:
        ...
        struct {
             ...
             * by finalize(). Kept as a standalone std::atomic<bool> 鈥?NOT a
             * bit-field 鈥?so the teardown guard has a well-defined happens-before
